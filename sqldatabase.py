...
# When backtesting, do u want to backtest from certain range, or entire data(>1h)
class SqlDatabase():
	"""
	...
	def __init__(self, symbol, timeframe, autoUpdate=True):
		...
		self.DATABASE = 'sqlite:///./data/%s.db' % self.DB_NAME
	...
	def access_db(self, column):
		...
		engine = create_engine(self.DATABASE, echo=False)
		cnx = engine.connect()
		return pd.read_sql_query("SELECT %s FROM %s" % (column, self.DB_NAME), cnx)
	...
	def ohlcv_range(self, columns, start, end):
		...
		ohlcv = pd.read_sql_query("SELECT %s FROM %s WHERE timestamp BETWEEN '%s' AND '%s'" % (columns,self.DB_NAME, start, end), cnx)
	...
	def resample_(self, tf="3D"):
		...
		# data = self.get_candles(count=500,range=False,reverseData=True)
		ohlcv.set_index(['timestamp'], inplace=True)
	...
	def candlestick_chart(self):
		"""
		Displays chart loading chart.html in browser
		"""
		ohlcv = self.get_ohlcv()
		# ohlcv.set_index('timestamp', inplace=False)
		ohlcv["timestamp"] = pd.to_datetime(ohlcv["timestamp"])
		...
		p = figure(x_axis_type="datetime", tools=TOOLS, plot_width=1425, title = self.symbol)
		p.xaxis.major_label_orientation = pi/4
		p.grid.grid_line_alpha=0.3

		p.segment(ohlcv.timestamp, ohlcv.high, ohlcv.timestamp, ohlcv.low, color="black")
		# p.vbar(ohlcv.timestamp[inc], w, ohlcv.open[inc], ohlcv.close[inc], fill_color="#FFFFFF", line_color="black")
		# p.vbar(ohlcv.timestamp[dec], w, ohlcv.open[dec], ohlcv.close[dec], fill_color="#000000", line_color="black")

		output_file("chart.html", title="XBTUSD")
...
test = SqlDatabase("XBTUSD", "1d", True)
print(test.get_first())
print(test.get_last())
# print(test.ohlcv_range("2019-09-18 17:00:00", "2020-06-16 05:00:00"))
# print(test.resample_())
...
# test.update_db()
# print(test.get_ohlcv())

# print(test.print_var())
# test.candlestick_chart()

# start = string_to_datetime("2015-09-24 00:00:00")
# print(test.get_candles(1000, start=start,reverseData=False))
# print(a["timestamp"])
# print(a["open"])

# print(test.get_timestamp()["timestamp"].iloc[-1])
# print(test.see_all().iloc[-1])
